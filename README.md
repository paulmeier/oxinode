...
| 15 | Messaging from the node: a spike | **measured, then dropped** — a Reticulum stack was built for the board and exchanged announces, packets and LXMF messages with a stock host, and the numbers are in the [notes](docs/phase-15-spike.md); the direction was then abandoned: this board is an RNode a companion app drives, not a node of its own, and the spike image and its dependencies are out of the tree |
