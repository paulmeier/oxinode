//! A canvas as a picture: 4x, with a pixel grid, in and out of PNG.
//!
...
//!
//! The picture is whatever size the canvas is: the board's frame comes out
//! 512 pixels square, a 128 × 64 panel 512 by 256.
...
/// A rendered image: RGB, row-major.
...
impl Image {
    ...
    /// over it.
    pub fn render(canvas: &impl Readable) -> Image {
        let width = canvas.width() * SCALE;
        let height = canvas.height() * SCALE;
        let mut pixels = vec![OFF; width * height];
        for y in 0..canvas.height() {
            for x in 0..canvas.width() {
                let colour = if canvas.pixel(x, y) { ON } else { OFF };
                for dy in 0..SCALE {
                    for dx in 0..SCALE {
                        let grid = dx == SCALE - 1 || dy == SCALE - 1;
                        pixels[(y * SCALE + dy) * width + x * SCALE + dx] =
    ...
    ///
    /// Anything that is not an 8-bit RGB image is refused rather than
    /// coerced, because the only way such a file ends up in the golden
    /// directory is by mistake, and a comparison against a coerced image
    /// reports a difference in the wrong place. The size is whatever the
    /// file says; a golden of the wrong size is [`Image::diff`]'s to report.
    ...
    /// identical. Two images of different sizes differ everywhere.
    pub fn diff(expected: &Image, actual: &Image) -> Option<Image> {
        if expected == actual {
            return None;
        }
        let same_size = (expected.width, expected.height) == (actual.width, actual.height);
        let pixels = expected
            .pixels
            .iter()
            .enumerate()
            .map(|(i, e)| {
                if same_size && *e == actual.pixels[i] {
                    [e[0] / 3, e[1] / 3, e[2] / 3]
                } else {
                    DIFF
                }
    ...
    /// How many pixels differ between two images. Two images of different
    /// sizes differ in every pixel of the larger.
    pub fn differing_pixels(a: &Image, b: &Image) -> usize {
        if (a.width, a.height) != (b.width, b.height) {
            return a.pixels.len().max(b.pixels.len());
        }
        a.pixels
            .iter()
            .zip(&b.pixels)
...
mod tests {
    ...
    /// The grid is drawn whether or not the pixel is lit, on a panel of any
    /// shape.
    #[test]
    fn the_grid_covers_the_whole_image() {
        let mut panel = Panel::new(Size::WIDE);
        monopanel::Canvas::fill(&mut panel, true);
        let image = Image::render(&panel);
    ...
    /// Encoding and decoding is lossless, at either size.
    #[test]
    fn png_round_trips() {
        let mut frame = Frame::new();
        ...
        let mut wide = Panel::new(Size::WIDE);
        monopanel::Canvas::rect(&mut wide, 1, 2, 3, 4, true);
        let image = Image::render(&wide);
        assert_eq!(Image::from_png(&image.to_png()).unwrap(), image);
    }

    /// A PNG of the wrong kind is refused, not coerced.
    #[test]
    ...
    fn a_diff_marks_where_and_only_where_they_differ() {
        ...
        let marked = diff.pixels.iter().filter(|&&p| p == DIFF).count();
    ...
    /// A golden of the wrong size is a difference, everywhere, and the
    /// diff picture is the expected image's size.
    #[test]
    fn images_of_different_sizes_differ_everywhere() {
        let square = Image::render(&Frame::new());
        let wide = Image::render(&Panel::new(Size::WIDE));
        let diff = Image::diff(&square, &wide).expect("a difference");
        assert_eq!((diff.width(), diff.height()), (512, 512));
        assert!(diff.pixels.iter().all(|&p| p == DIFF));
        assert_eq!(Image::differing_pixels(&square, &wide), 512 * 512);
        assert_eq!(Image::differing_pixels(&wide, &square), 512 * 512);
    }
