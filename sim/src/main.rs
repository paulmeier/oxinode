...
//!
//! ```text
//! oxinode-sim render [--script S] [--sample N] [--text] -o out.png
//! oxinode-sim steps  [--script S] [--sample N] --out DIR
//! oxinode-sim tty    [--script S] [--sample N] [--braille | --half]
...
usage: oxinode-sim <command> [options]
  ...
  render [--script S] [--sample N] [--text] -o FILE.png
  ...
  steps  [--script S] [--sample N] --out DIR
      The same, writing one PNG per step: 00-start.png, 01-right.png, ...
  tty    [--script S] [--sample N] [--braille | --half]
...
with word*N for repeats and # for comments. --sample N gives the screen N
numbered lines of content, enough to scroll if N is large.
";
...
fn run(command: Command) -> Result<(), String> {
    match command {
        ...
        Command::Render {
            ...
            let mut scene = Scene::new().with_sample_lines(sample);
...
mod tests {
    ...
    fn render_needs_somewhere_to_go() {
        ...
        assert_eq!(
            parse("render --script right --sample 3 -o x.png").unwrap(),
            Command::Render {
                script: "right".into(),
                sample: 3,
                text: false,
    ...
    fn a_bad_script_is_refused_up_front() {
        ...
        assert!(parse("render --sample lots -o x.png").is_err());
